...
public class Manager {
    ...
    public static final int FECHA_FIN_PER_MILLIS = 1440;
    public static final int DIGITOS_NUM_SS = 12;
    public static final int NUMEROS_DNI = 8;
    public static final int CHARS_EN_DNI = 9;
    public static final int FACTOR_CALCULO_LETRA_DNI = 23;
	...
	public char calcularLetraDni(int dniNum) {
	    ...
	    int modulo = dniNum % FACTOR_CALCULO_LETRA_DNI;
	...
	public void comprobarDni(String dni) throws Exception {
		int dniNum;
		if (dni.length() > CHARS_EN_DNI) {
			throw new Exception("El dni es incorrecto");
		}
		for (int i = 0; i < dni.length(); i++) {
			if (i < NUMEROS_DNI
					&& !Character.isDigit(dni.charAt(i))) {
				throw new Exception("El dni es incorrecto");
			}
			if (i == NUMEROS_DNI
					&& !Character.isLetter(dni.charAt(i))) {
				throw new Exception("El dni es incorrecto");
			}
		}
		dniNum = Integer.parseInt(dni.substring(0, NUMEROS_DNI));
		char letra = calcularLetraDni(dniNum);
		if (Character.toUpperCase(letra)
			!= Character.toUpperCase(dni.charAt(NUMEROS_DNI))) {
			throw new Exception("El dni es incorrecto");
		}
	}
	...
	private void comprobarNSS(String numSS) throws Exception{
		if (numSS.length() != DIGITOS_NUM_SS) {
			throw new Exception("El numero de seguridad"
					+ " social no es correcto");
		}
		for (int i = 0; i < numSS.length(); i++) {
			if (!Character.isDigit(numSS.charAt(i))) {
				throw new Exception("El número de seguridad"
						+ " social no es correcto");
			}
		}
	}
	 ...
	 */
	private void controlarSolapamiento(String dniPaciente,
			String dniMedico, Date fechaCita) throws Exception {
		List<Cita> citas = citaRepo.findByDniPaciente(dniPaciente);
		Medico medico =
				medicoRepo.
				findByDni(Cifrador.cifrar(dniMedico));
		Especialidad especialidad =
				especialidadRepo.
				findCustomEspecialidad(medico.
						getIdEspecialidad());
		int duracion = especialidad.getDuracionCita();
		Date fechaCitaPlusDuracion =
				new Date(fechaCita.getTime()
					+ (duracion
					* ONE_MINUTE_IN_MILLIS));
		for (int i = 0; i < citas.size(); i++) {
			Date fechaSolapada = citas.get(i).getFecha();
			Date fechaSolapadaPlusDuracion =
					new Date(fechaSolapada.getTime()
						+ (duracion
						* ONE_MINUTE_IN_MILLIS));
			if ((fechaCita.after(fechaSolapada)
					&& fechaCita.
					before(fechaSolapadaPlusDuracion))
					|| (fechaCita.before(fechaSolapada)
						&& fechaCitaPlusDuracion.
							after(fechaSolapada))) {
				throw new Exception("Las fechas se solapan, "
						+ "cita incorrecta");
			}
			if (fechaCita.equals(fechaSolapada)
					|| fechaCita.
					equals(fechaSolapadaPlusDuracion)) {
				throw new Exception("Las fechas no pueden"
						+ " ser iguales");
			}
		}
		...
		for (int i = 0; i < citas.size(); i++) {
			...
			if (fechaCita.equals(fechaSolapada)
					|| fechaCita.
					equals(fechaSolapadaPlusDuracion)) {
				throw new Exception("Las fechas no"
						+ " pueden ser iguales");
			}
		}
	}
	 ...
	 */
	public Usuario register(String dni,	String nombre,
			String apellidos, String contrs, String numSS,
			int idEspecialidad) throws Exception {
		if (dni == null || nombre == null || apellidos == null
				|| contrs == null || numSS == null) {
			throw new Exception("No debe haber campos vacios");
		}
		if (dni.equals("") || nombre.equals("") || apellidos.equals("")
				|| contrs.equals("") || numSS.equals("")) {
			throw new Exception("No debe haber campos vacios");
		}
	 ...
	 * @throws Exception si la contraseña es incorrecta.
	 */
	public List<Cita> getCitasMedico(String dni,
		...
		Date fechaDiaFin =
				new Date(fechaDia.getTime()
						+ (FECHA_FIN_PER_MILLIS
						* ONE_MINUTE_IN_MILLIS));
		for (int i = 0; i < listaCitas.size(); i++) {
			Date fechaCita = listaCitas.get(i).getFecha();
			if ((fechaCita.after(fechaDia)
					&& fechaCita.before(fechaDiaFin))
					|| fechaCita.equals(fechaDia)) {
	 ...
	 */
	public Cita pedirCita(String dniPaciente, String fecha,
			String especialidad) throws Exception {
		PacienteMedico pacienteMed =
				pacienteMedicoRepo.findCustomMedico(dniPaciente,
						especialidad);
		...
		if (fechaCita.compareTo(sysdate) < 0) {
	          throw new Exception("La fecha de la cita"
	          		+ " no puede ser pasada");
		}
		controlarSolapamiento(dniPaciente, dniMedico, fechaCita);
		Cita cita = new Cita(fechaCita, dniMedico,
	 ...
	 * @throws Exception si los datos son incorrectos.
	 */
	public List<Date> getCitas(String dniPaciente,
			String especialidad) throws Exception {
		PacienteMedico pacienteMed =
				pacienteMedicoRepo.
					findCustomMedico(dniPaciente,
							especialidad);
		String dniMedico = pacienteMed.getDniMedico();
		List<Cita> citas =
				citaRepo.findByDniPaciente(dniPaciente);
	 ...
	 */
	public void eliminarCitas(String dniPaciente,
			String fecha,
			String especialidad) throws ParseException {
		PacienteMedico pacienteMed =
				pacienteMedicoRepo.findCustomMedico(dniPaciente,
						especialidad);
		String dniMedico = pacienteMed.getDniMedico();
		Date fechaCita =
				...
				parse(fecha);
		List<Cita> lista =
				citaRepo.deleteCustomCita(dniPaciente,
						dniMedico, fechaCita);
	 ...
	 * @throws ParseException si el formato de la fecha es incorrecto.
	 */
	public boolean existeCita(String dniPaciente,
			String especialidad,
		...
		Date fechaCita =
				new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").
					parse(fecha);
		return citaRepo.existCustomCita(dniPaciente,
				dniMedico, fechaCita);
	}
	 ...
	 */
	public Cita modificarCita(String dniPaciente,
			String especialidad, String fechaActual,
			String fechaModificar) throws ParseException {
		PacienteMedico pacienteMed =
				pacienteMedicoRepo.
					findCustomMedico(dniPaciente,
							especialidad);
		...
		Date fechaNueva =
				new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").
					parse(fechaModificar);
		List<Cita> list =
				citaRepo.deleteCustomCita(dniPaciente,
						dniMedico, fechaCita);
		Cita cita =
				new Cita(fechaNueva, dniMedico,
						dniPaciente, especialidad);
	...
	/**
	 * Crea una especialidad con los datos dados y la inserta en la BBDD.
	 ...
	 */
	public Especialidad crearEspecialidad(String nombrEspecialidad,
			String tiempoCita, String horaInicio,
			String horaFin) throws Exception {
		Date horaInicioDate =
				new SimpleDateFormat("HH:mm").parse(horaInicio);
		...
		if (tiempoCitaInt <= 0 || tiempoCitaInt > MINUTOS_POR_HORA) {
			throw new Exception("La duracion de la cita"
					+ " de la especialidad debe ser mayor "
					+ "que 0 y menor o igual que 60");
		}
		Especialidad especialidad =
				new Especialidad(nombrEspecialidad,
						tiempoCitaInt, horaInicioDate,
							horaFinDate);
		if (especialidadRepo.
				findCustomEspecialidad(nombrEspecialidad) !=
					null) {
	...
	public Especialidad eliminarEspecialidad(String nombreEspecialidad) {
		Especialidad lista =
				especialidadRepo.
					findCustomEspecialidad(nombreEspecialidad);
	 ...
	 * @throws Exception.
	 */
	public Especialidad modificarEspecialidad(String nombre,
			String duracionOld, String horaInicioOld,
			String horaFinOld,
			String duracionNew,
			String horaInicioNew,
			String horaFinNew) throws Exception {
		Especialidad especialidad =
				especialidadRepo.findCustomEspecialidad(nombre);
		...
		Date horaInicioDate =
				new SimpleDateFormat("HH:mm").
					parse(horaInicioNew);
		Date horaFinDate =
				new SimpleDateFormat("HH:mm").parse(horaFinNew);
		numerosNombre(nombre);
		if (duracionNewInt <= 0 || duracionNewInt > MINUTOS_POR_HORA) {
			throw new Exception("La duracion de la cita "
					+ "de la especialidad debe ser mayor "
					+ "que 0 y menor o igual que 60");
		}
		Especialidad esp =
				new Especialidad(nombre, duracionNewInt,
						horaInicioDate, horaFinDate);
	 ...
	 */
	public Medico crearMedico(String dniMedico,
			String especialidad) throws Exception {
		Usuario user = userRepo.findByDni(Cifrador.cifrar(dniMedico));
		if (user == null) {
			throw new Exception("El medico debe estar "
					+ "registrado como usuario");
		}
		if (especialidadRepo.
				findCustomEspecialidad(especialidad) == null) {
			throw new Exception("Especialidad no existente");
		}
		Medico medico = new Medico(Cifrador.descifrar(user.getDni()),
				Cifrador.descifrar(user.getNombre()),
				Cifrador.descifrar(user.getApellidos()),
				user.getContrs(), especialidad);
	...
	public Medico eliminarMedico(String dniMedico) throws Exception {
		Medico medico = medicoRepo.
				findByDni(Cifrador.cifrar(dniMedico));
		List<PacienteMedico> pacMed = pacienteMedicoRepo.
				findCustomDniMedico(dniMedico);
	 ...
	 * @throws Exception.
	 */
	public PacienteMedico crearMedicoPaciente(String dniPaciente,
			String dniMedico) throws Exception {
		Medico medico = medicoRepo.
				findByDni(Cifrador.cifrar(dniMedico));
		String especialidad = medico.getIdEspecialidad();
		if (userRepo.findByDni(Cifrador.cifrar(dniPaciente)) == null) {
			throw new Exception("El Dni no corresponde"
					+ " con un paciente");
		}
		if (pacienteMedicoRepo.
				findCustomMedico(dniPaciente,
						especialidad) == null) {
			PacienteMedico pacMed = new
					PacienteMedico(dniPaciente, dniMedico,
							especialidad);
			pacienteMedicoRepo.insert(pacMed);
			return pacMed;
		} else {
			throw new Exception("El paciente ya tiene "
					+ "un medico asignado para la "
					+ "especialidad" + especialidad);
		}
	}
	 ...
	 * @throws Exception
	 */
	public PacienteMedico eliminarPacienteMedico(String dniPaciente,
			String dniMedico) throws Exception {
		String especialidad = medicoRepo.
				findByDni(Cifrador.cifrar(dniMedico)).
					getIdEspecialidad();
		PacienteMedico pacMed = pacienteMedicoRepo.
				findCustomMedico(dniPaciente, especialidad);
